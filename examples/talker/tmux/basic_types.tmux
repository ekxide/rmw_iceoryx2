#!/usr/bin/env bash
# ROS 2 talker -> ROS 2 listener (BasicTypes).
set -eo pipefail
...
tmux send-keys -t "$SESSION_NAME" "$prelude; ros2 run rmw_iceoryx2_talker_demo_nodes listener_basic_types" C-m
tmux split-window -h -t "$SESSION_NAME" "$bash_bin"
tmux send-keys -t "$SESSION_NAME" "$prelude; ros2 run rmw_iceoryx2_talker_demo_nodes talker_basic_types" C-m
